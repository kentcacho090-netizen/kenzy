...
function renderConversations(){ const el=document.querySelector('.ai-v3-conversations'); if(!el)return; const active=activeConversation(); const rows=conversations(); el.innerHTML=rows.length?rows.map(c=>`<div class="ai-v3-conv ${c.id===active?'active':''}"><button type="button" data-open="${c.id}"><strong>${escapeHtml(c.title)}</strong><small>${escapeHtml(c.messages?.slice().reverse().find(m=>m.role==='user')?.content||'New conversation')}</small></button><button type="button" data-delete="${c.id}" aria-label="Delete conversation">×</button></div>`).join(''):'<div class="ai-v3-empty">No conversations yet.</div>'; el.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>loadConversation(b.dataset.open)); el.querySelectorAll('[data-delete]').forEach(b=>b.onclick=async()=>{const id=b.dataset.delete; await Promise.resolve(); const next=rows.filter(x=>x.id!==id); save(CONV_KEY,next); if(activeConversation()===id) save(ACTIVE_KEY,next[0]?.id||null); if(!next.length) ensureConversation(); loadConversation(activeConversation());}); }

function renderFiles(){ const el=document.querySelector('.ai-v3-files'); if(!el)return; const rows=load(META_KEY,[]); const selected=new Set(load('studyken-ai-selected-v3',[])); el.innerHTML=rows.length?rows.map(f=>`<div class="ai-v3-file ${selected.has(f.id)?'selected':''}"><input type="checkbox" data-select="${f.id}" ${selected.has(f.id)?'checked':''}><span class="ai-v3-type">${f.type.includes('presentation')||f.type.includes('powerpoint')?'PPT':f.type.includes('pdf')?'PDF':'IMG'}</span><div><strong>${escapeHtml(f.name)}</strong><small>${formatSize(f.size)}</small></div><button type="button" data-remove="${f.id}">×</button></div>`).join(''):'<div class="ai-v3-empty">No study files yet.</div>'; el.querySelectorAll('[data-select]').forEach(i=>i.onchange=()=>{const s=new Set(load('studyken-ai-selected-v3',[])); i.checked?s.add(i.dataset.select):s.delete(i.dataset.select); save('studyken-ai-selected-v3',[...s]); renderFiles(); renderAttachments();}); el.querySelectorAll('[data-remove]').forEach(b=>b.onclick=async()=>{await deleteFile(b.dataset.remove); save(META_KEY,rows.filter(x=>x.id!==b.dataset.remove)); save('studyken-ai-selected-v3',load('studyken-ai-selected-v3',[]).filter(x=>x!==b.dataset.remove)); renderFiles(); renderAttachments();}); }
function renderAttachments(){ const el=document.querySelector('.ai-v3-attachments'); if(!el)return; const rows=load(META_KEY,[]); const selected=new Set(load('studyken-ai-selected-v3',[])); el.innerHTML=rows.filter(f=>selected.has(f.id)).map(f=>`<span>📎 ${escapeHtml(f.name)} <button type="button" data-unattach="${f.id}">×</button></span>`).join(''); el.querySelectorAll('[data-unattach]').forEach(b=>b.onclick=()=>{save('studyken-ai-selected-v3',load('studyken-ai-selected-v3',[]).filter(x=>x!==b.dataset.unattach));renderFiles();renderAttachments();}); }

function renderMessages(c){ const el=document.querySelector('.ai-v3-messages'); if(!el)return; el.innerHTML=c.messages.map(m=>`<article class="ai-v3-message ${m.role}"><div class="ai-v3-avatar">${m.role==='user'?'KC':'K'}</div><div class="ai-v3-bubble"><div class="ai-v3-label">${m.role==='user'?'You':'StudyKen'}</div>${m.content?`<div>${formatText(m.content)}</div>`:''}${m.attachments?.length?`<div class="ai-v3-message-files">${m.attachments.map(a=>`<span>📎 ${escapeHtml(a.name)}</span>`).join('')}</div>`:''}</div></article>`).join(''); el.scrollTop=el.scrollHeight; }
...
async function addFiles(list){ const files=Array.from(list||[]); if(!files.length)return; const bad=files.find(f=>!ALLOWED.has(f.type)); if(bad){setStatus(`Unsupported file type: ${bad.name}`,'error');return;} const meta=load(META_KEY,[]); const incoming=files.reduce((n,f)=>n+f.size,0); if(totalSavedBytes(meta)+incoming>MAX_FILE_BYTES){setStatus('Keep saved study files at or below 25 MB combined.','error');return;} const selected=new Set(load('studyken-ai-selected-v3',[])); for(const file of files){const id=uid(); await putFile({id,name:file.name,type:file.type,size:file.size,blob:file}); meta.unshift({id,name:file.name,type:file.type,size:file.size,addedAt:Date.now()}); selected.add(id);} save(META_KEY,meta.slice(0,30)); save('studyken-ai-selected-v3',[...selected].slice(0,10)); renderFiles(); renderAttachments(); setStatus(`${files.length} file${files.length>1?'s':''} ready to attach.`,'success'); }

async function prepareSelected(){ const ids=load('studyken-ai-selected-v3',[]); const inline=[]; const blobs=[]; const attachments=[]; for(const id of ids){ const f=await getFile(id); if(!f?.blob)continue; attachments.push({id:f.id,name:f.name,type:f.type,size:f.size}); if(f.size<=INLINE_LIMIT){ inline.push({name:f.name,mimeType:f.type,data:await fileToBase64(f.blob)}); } else { const {upload}=await import('@vercel/blob/client'); const uploaded=await upload(`kenzy-material/${uid()}-${f.name.replace(/[^a-zA-Z0-9._-]+/g,'-').slice(0,100)}`,f.blob,{access:'private',handleUploadUrl:'/api/blob-upload',multipart:f.size>5*1024*1024}); blobs.push({pathname:uploaded.pathname,name:f.name,mimeType:f.type,size:f.size}); } } return {inline,blobs,attachments}; }

async function send(e){ e.preventDefault(); const ta=document.querySelector('.ai-v3-form textarea'); const text=ta?.value.trim()||''; const selected=load('studyken-ai-selected-v3',[]); if(!text&&!selected.length)return; const c=ensureConversation(); const prepared=await prepareSelected(); const user={role:'user',content:text||'Please analyze my attached study material.',attachments:prepared.attachments}; c.messages.push(user); if(c.title==='New chat'){const base=text||prepared.attachments[0]?.name||'Study material';c.title=base.length>45?`${base.slice(0,45)}…`:base;} c.updatedAt=Date.now(); persist(c); save('studyken-ai-selected-v3',[]); renderConversations(); renderMessages(c); renderAttachments(); ta.value=''; setBusy(true); setStatus(prepared.attachments.length?`Kenzy is checking ${prepared.attachments.length} attached file${prepared.attachments.length>1?'s':''}…`:'Kenzy is thinking…','working'); try{ const body={messages:c.messages.slice(-8).map(m=>({role:m.role,content:m.content})),files:prepared.inline,blobFiles:prepared.blobs}; const response=await fetch('/api/ai-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); const data=await response.json().catch(()=>({})); if(!response.ok)throw new Error(data.error||'AI request failed.'); c.messages.push({role:'assistant',content:data.reply}); c.updatedAt=Date.now(); persist(c); renderConversations(); renderMessages(c); setStatus('Ready.','success'); }catch(error){ c.messages.push({role:'assistant',content:`Sorry, I couldn't answer that. ${error.message||'Please try again.'}`}); persist(c); renderMessages(c); setStatus(error.message||'AI request failed.','error'); }finally{setBusy(false);} }
function setBusy(busy){const btn=document.querySelector('.ai-v3-send');const provider=document.querySelector('.ai-v3-provider');if(btn){btn.disabled=busy;btn.textContent=busy?'Thinking…':'Send ↵';}if(provider)provider.innerHTML=busy?'<i class="busy"></i> Working…':'<i></i> Fast AI';}
