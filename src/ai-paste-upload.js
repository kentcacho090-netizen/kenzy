...
function renderPastedUi(){
  ...
  if(filesEl){
    ...
    filesEl.querySelectorAll('[data-paste-delete]').forEach(button=>button.addEventListener('click',()=>{write(PASTE_META,files.filter(f=>f.id!==button.dataset.pasteDelete));write(PASTE_SELECTED,[...selected].filter(id=>id!==button.dataset.pasteDelete));renderPastedUi();}));
  }
...
function boot(){bindPaste();injectStyles();injectNotice();const observer=new MutationObserver(()=>{injectNotice();renderPastedUi();});observer.observe(document.body,{childList:true,subtree:true});}
