...
(function () {
  ...
  function looksLikePptx(base64) {
    try {
      ...
      const bytes = Uint8Array.from(atob(clean.slice(0, 4096)), c => c.charCodeAt(0));
  ...
  function looksLikeLegacyPpt(base64) {
    try {
      ...
      const bytes = Uint8Array.from(atob(clean.slice(0, 16)), c => c.charCodeAt(0));
  ...
  function patchFile(file) {
    ...
    const isPpt = lower.endsWith('.ppt') || lower.endsWith('.pptx') || file.type === PPT || file.type === PPTX;
    if (!isPpt) return;
    try {
      Object.defineProperty(file, '__kenzyOriginalMime', { configurable: true, value: mimeFor(file.name, file.type) });
  ...
  window.fetch = async function presentationAwareFetch(input, init = {}) {
    ...
    if (total > MAX) return originalFetch(input, { ...init, body: JSON.stringify({ ...body, files: normalized }) });
