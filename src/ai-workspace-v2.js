...
function renderFiles() {
  ...
  el.querySelectorAll('[data-file-delete]').forEach(btn => btn.onclick = async () => { await deleteFile(btn.dataset.fileDelete); save(FILE_META_KEY, files.filter(f => f.id !== btn.dataset.fileDelete)); save('kenzy-ai-selected-files-v2', load('kenzy-ai-selected-files-v2', []).filter(x => x !== btn.dataset.fileDelete)); renderFiles(); renderAttachments(); });
}
function renderAttachments() { const el = document.querySelector('.ai-v2-attachments'); if (!el) return; const files = load(FILE_META_KEY, []); const selected = new Set(load('kenzy-ai-selected-files-v2', [])); el.innerHTML = files.filter(f => selected.has(f.id)).map(f => `<span>📎 ${escapeHtml(f.name)} <button type="button" data-unattach="${f.id}">×</button></span>`).join(''); el.querySelectorAll('[data-unattach]').forEach(b => b.onclick = () => { save('kenzy-ai-selected-files-v2', load('kenzy-ai-selected-files-v2', []).filter(x => x !== b.dataset.unattach)); renderFiles(); renderAttachments(); }); }
...
async function addFiles(fileList) { const files = Array.from(fileList || []); if (!files.length) return; const total = files.reduce((n,f)=>n+f.size,0); if (total > MAX_FILE_BYTES) return alert('Keep the selected AI files under 3 MB combined.'); const allowed = ['application/pdf','image/png','image/jpeg','image/webp']; if (files.some(f=>!allowed.includes(f.type))) return alert('Use PDF, PNG, JPG, or WebP files only.'); const meta = load(FILE_META_KEY, []); for (const file of files) { const fileId=id(); await putFile({ id:fileId, name:file.name, type:file.type, size:file.size, blob:file }); meta.unshift({id:fileId,name:file.name,type:file.type,size:file.size,addedAt:Date.now()}); } save(FILE_META_KEY, meta.slice(0,30)); renderFiles(); }
async function send(event) { event.preventDefault(); const ta=document.querySelector('.ai-v2-form textarea'); const text=ta?.value.trim(); if(!text)return; const conversation=ensureConversation(); const selected=load('kenzy-ai-selected-files-v2', []); const meta=load(FILE_META_KEY, []); const files=[]; for(const fid of selected){ const f=await getFile(fid); if(f?.blob) files.push({name:f.name,mimeType:f.type,data:await blobToBase64(f.blob)}); }
