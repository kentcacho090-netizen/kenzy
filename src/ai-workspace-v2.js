...
function renderMessages(conversation){const el=document.querySelector('.ai-v2-messages');if(!el)return;el.innerHTML=conversation.messages.map(m=>`<article class="ai-v2-message ${m.role}"><div class="ai-v2-avatar">${m.role==='user'?'KC':'K'}</div><div class="ai-v2-bubble"><div class="ai-v2-message-label">${m.role==='user'?'You':'Kenzy'}</div><div class="ai-v2-content">${formatText(m.content)}</div></div></article>`).join('');el.scrollTop=el.scrollHeight;}
