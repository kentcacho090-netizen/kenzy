  ...
  page.innerHTML = `
    <div class="ai-v2-shell">
      <aside class="ai-v2-sidebar">
        <div class="ai-v2-side-head"><div><div class="eyebrow">KENZY AI</div><h3>Workspace</h3></div><button class="ai-v2-new">＋ New chat</button></div>
...
function renderConversations() {
  const el = document.querySelector('.ai-v2-conversations'); if (!el) return;
  const active = getActive(); const rows = getConversations();
  el.innerHTML = rows.length ? rows.map(c => `<div class="ai-v2-conv ${c.id===active?'active':''}" data-conv="${c.id}"><button class="ai-v2-conv-open" data-open="${c.id}"><strong>${escapeHtml(c.title)}</strong><small>${escapeHtml(lastText(c))}</small></button><button class="ai-v2-conv-menu" data-delete="${c.id}" title="Delete conversation">×</button></div>`).join('') : '<div class="ai-v2-empty">No conversations yet.</div>';
  el.querySelectorAll('[data-open]').forEach(b => b.onclick = () => loadConversation(b.dataset.open));
  el.querySelectorAll('[data-delete]').forEach(b => b.onclick = (e) => { e.stopPropagation(); deleteConversation(b.dataset.delete); });
}
function lastText(c) { const m = [...c.messages].reverse().find(x => x.role === 'user'); return m?.content || 'New conversation'; }
function renderFiles() {
  const el = document.querySelector('.ai-v2-files'); if (!el) return;
  const files = load(FILE_META_KEY, []); const active = new Set(load('kenzy-ai-selected-files-v2', []));
  el.innerHTML = files.length ? files.map(f => `<label class="ai-v2-file ${active.has(f.id)?'selected':''}"><input type="checkbox" data-file-select="${f.id}" ${active.has(f.id)?'checked':''}/><span class="ai-v2-file-type">${f.type.includes('pdf')?'PDF':'IMG'}</span><span><strong>${escapeHtml(f.name)}</strong><small>${formatSize(f.size)}</small></span><button type="button" data-file-delete="${f.id}" title="Remove">×</button></label>`).join('') : '<div class="ai-v2-empty">No study files yet.<br/>Add a PDF or image to ask Kenzy about it.</div>';
  el.querySelectorAll('[data-file-select]').forEach(input => input.onchange = () => { const selected = new Set(load('kenzy-ai-selected-files-v2', [])); input.checked ? selected.add(input.dataset.fileSelect) : selected.delete(input.dataset.fileSelect); save('kenzy-ai-selected-files-v2', [...selected]); renderFiles(); renderAttachments(); });
  el.querySelectorAll('[data-file-delete]').forEach(btn => btn.onclick = async (e) => { e.preventDefault(); e.stopPropagation(); await deleteFile(btn.dataset.fileDelete); save(FILE_META_KEY, files.filter(f => f.id !== btn.dataset.fileDelete)); save('kenzy-ai-selected-files-v2', load('kenzy-ai-selected-files-v2', []).filter(x => x !== btn.dataset.fileDelete)); renderFiles(); renderAttachments(); });
}
...
function renderMessages(conversation, streamingId = '') { const el = document.querySelector('.ai-v2-messages'); if (!el) return; el.innerHTML = conversation.messages.map((m,i) => `<article class="ai-v2-message ${m.role}"><div class="ai-v2-avatar">${m.role==='user'?'KC':'K'}</div><div class="ai-v2-bubble"><div class="ai-v2-message-label">${m.role==='user'?'You':'Kenzy'}</div><div class="ai-v2-content">${formatText(m.content)}${streamingId===`${i}`?'<span class="ai-v2-cursor">▌</span>':''}</div></div></article>`).join(''); el.scrollTop = el.scrollHeight; }
...
async function addFiles(fileList) {
  ...
  const existing = load(FILE_META_KEY, []);
  const selectedBytes = existing.reduce((n,f)=>n+f.size,0);
  const incomingBytes = files.reduce((n,f)=>n+f.size,0);
  if (selectedBytes + incomingBytes > MAX_FILE_BYTES) return alert('Keep your saved AI files under 3 MB total for now.');
  const allowed = ['application/pdf','image/png','image/jpeg','image/webp'];
  if (files.some(f=>!allowed.includes(f.type))) return alert('Use PDF, PNG, JPG, or WebP files only.');
  const meta = [...existing]; const selected = new Set(load('kenzy-ai-selected-files-v2', []));
  for (const file of files) { const fileId=id(); await putFile({ id:fileId, name:file.name, type:file.type, size:file.size, blob:file }); meta.unshift({id:fileId,name:file.name,type:file.type,size:file.size,addedAt:Date.now()}); selected.add(fileId); }
  save(FILE_META_KEY, meta.slice(0,30)); save('kenzy-ai-selected-files-v2', [...selected].slice(0,10)); renderFiles(); renderAttachments();
}
async function send(event) {
  event.preventDefault();
  const ta=document.querySelector('.ai-v2-form textarea'); const text=ta?.value.trim(); if(!text)return;
  const conversation=ensureConversation(); const selected=load('kenzy-ai-selected-files-v2', []); const files=[];
  for(const fid of selected){ const f=await getFile(fid); if(f?.blob) files.push({name:f.name,mimeType:f.type,data:await blobToBase64(f.blob)}); }
  conversation.messages.push({role:'user',content:text}); if(conversation.title==='New chat') conversation.title=text.length>45?`${text.slice(0,45)}…`:text; conversation.updatedAt=Date.now(); persistConversation(conversation); renderConversations(); renderMessages(conversation); ta.value=''; setBusy(true);
...
function bind(){
  document.querySelector('.ai-v2-new').onclick=()=>{ const c={id:id(),title:'New chat',messages:[{role:'assistant',content:'New chat started. What are you studying?'}],updatedAt:Date.now()}; save('kenzy-ai-active-v2',c.id); save(CONV_KEY,[c,...getConversations()].slice(0,50)); loadConversation(c.id); };
  document.querySelector('.ai-v2-form').addEventListener('submit',send);
  document.querySelector('.ai-v2-file-input').addEventListener('change',e=>{addFiles(e.target.files); e.target.value='';});
  document.querySelector('.ai-v2-compose-file-input').addEventListener('change',e=>{addFiles(e.target.files); e.target.value='';});
  document.querySelectorAll('[data-quick]').forEach(b=>b.onclick=()=>{const ta=document.querySelector('.ai-v2-form textarea');ta.value=b.dataset.quick;ta.focus();});
  const ta=document.querySelector('.ai-v2-form textarea');
  ta.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();document.querySelector('.ai-v2-form').requestSubmit();}});
}
...
let observer;
function boot(){if(observer)return; observer=new MutationObserver(render); observer.observe(document.body,{childList:true,subtree:true}); render();}
