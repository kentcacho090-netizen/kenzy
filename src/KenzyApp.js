...
const EMPTY_NOTE = { id: null, title: 'Untitled note', content: '', updatedAt: null };
...
function App() {
  ...
  function deleteQuiz(id) {
    ...
    if (activeQuiz && activeQuiz.id === id) goHome();
  }
...
function NavigationMenu({ page, navigate, onClose }) {
  ...
  return (
    <aside className="side-menu">
      ...
      <div className="menu-section">{item('home', '⌂', 'Home', 'Your study dashboard')}</div>
  ...
  return (
    <section className="page home-page">
      <div className="hero-copy">
        ...
        <h1>
          Everything you need to <span>study smarter.</span>
        </h1>
        <p>Generate quizzes, ask AI for help, build study notes, and find useful study applications from one place.</p>
        <div className="hero-actions">
          <button className="button primary" onClick={() => navigate('quiz-maker')}>
            ＋ Create a quiz
          </button>
          <button className="button secondary" onClick={() => navigate('ai')}>
            ✦ Ask Kenzy AI
          </button>
        </div>
      </div>
        ...
        <section className="workspace-card">
          <div className="card-heading">
            <div>
              ...
              <small>
                {quizzes.length} quizzes · {notes.length} notes
              </small>
            </div>
          ...
          {quizzes.length === 0 && notes.length === 0 ? (
            <div className="empty-mini">
              ...
              <span>Open the menu or use the buttons above to start.</span>
            </div>
          ) : (
            <div className="saved-list">
              {quizzes.slice(0, 2).map((quiz) => (
                <button key={quiz.id} className="saved-row" onClick={() => navigate('manage')}>
                  <span>✦ {quiz.title}</span>
                  <small>{quiz.questions.length} questions</small>
                </button>
              ))}
              {notes.slice(0, 2).map((note) => (
                <button key={note.id} className="saved-row" onClick={() => navigate('notes')}>
        ...
        <section className="workspace-card applications-preview">
          <div className="card-heading">
            <div>
              ...
              <small>Tools you can download later</small>
            </div>
          ...
          <div className="app-preview-row">
            ...
            <div>
              ...
              <small>Curated study tools and download links.</small>
            </div>
            <span>→</span>
          </div>
        </section>
  ...
  async function generate() {
    ...
    try {
      ...
      const response = await fetch('/api/generate-quiz', {
        ...
        body: JSON.stringify({ files: encoded, count, suggestion: suggestion.trim().slice(0, 1000) }),
      });
  ...
  return (
    <section className="page narrow-page">
      ...
      <div className="section-heading">
        <div className="eyebrow">QUIZ MAKER</div>
        <h2>Create a quiz</h2>
        <p>Use PDFs or images, choose from 1–100 questions, and practice against a timer.</p>
      </div>
      <label className={`upload-box ${files.length ? 'has-files' : ''}`}>
        ...
        <small>
          {files.length
            ? `${Math.ceil(files.reduce((a, f) => a + f.size, 0) / 1024)} KB total · ${files.map((f) => f.name).join(', ')}`
            : 'PDF, PNG, JPG, or WebP · maximum 3 MB combined'}
        </small>
      ...
      <div style={{ marginTop: 16, padding: 18, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18 }}>
        ...
        <p style={{ margin: '6px 0 12px', color: 'var(--muted)', lineHeight: 1.5, fontSize: 13 }}>
          Add extra instructions about difficulty, question style, focus, or problem-solving.
        </p>
        <textarea
          value={suggestion}
      ...
      <div className="settings-grid">
        <Range label="Questions" value={count} min={1} max={100} onChange={setCount} />
        <Range label="Time limit" value={minutes} min={1} max={180} suffix="min" onChange={setMinutes} />
      </div>
      {error && <div className="error-box">{error}</div>}
      <button className="button primary full-width" disabled={!files.length || busy} onClick={generate}>
        {busy ? 'Generating…' : 'Generate quiz'}
      </button>
      <p className="privacy-note">Files are sent to Kenzy server only when you press Generate quiz.</p>
      ...
      <div className="generation-panel">
        ...
        <div className="generation-steps">
          {stages.map((item, index) => (
            <div className={`generation-step ${index < stage ? 'complete' : index === stage ? 'active' : 'waiting'}`} key={item[0]}>
        ...
        <div className="generation-safe-note">Kenzy shows status updates only; private model reasoning is never displayed.</div>
      </div>
    </div>
  );
}
...
function QuizPage({ quiz, onExit, onFinish }) {
  ...
  function submit() {
    ...
    onFinish({
      ...
      answers,
      timeSpent: Math.max(0, quiz.timeLimit * 60 - seconds),
    });
  }
  ...
  return (
    <section className="page quiz-page">
      <div className="quiz-toolbar">
        ...
        <div className={seconds < 60 ? 'timer danger' : 'timer'}>
      ...
      <div className="question-count">
        Question {index + 1} <span>of {quiz.questions.length}</span>
      </div>
      ...
      <div className="quiz-navigation">
        <button className="button secondary" disabled={index === 0} onClick={() => setIndex((value) => value - 1)}>
          ← Previous
        </button>
...
function ResultsPage({ quiz, onHome, onRetry }) {
  const attempt = quiz.lastAttempt || (quiz.attempts && quiz.attempts[quiz.attempts.length - 1]);
  const [filter, setFilter] = useState('all');
  const items = useMemo(
    () =>
      quiz.questions.map((q, i) => {
        const selected = (attempt && attempt.answers && attempt.answers[i]) ?? -1;
        return { ...q, number: i + 1, selected, correct: selected === q.correctIndex, unanswered: selected < 0 };
      }),
  ...
  const shown = items.filter(
    (item) =>
      filter === 'all' ||
      (filter === 'correct' && item.correct) ||
      (filter === 'wrong' && !item.correct && !item.unanswered) ||
      (filter === 'unanswered' && item.unanswered)
  );
  const incorrect = quiz.questions.length - ((attempt && attempt.correct) || 0);

  return (
    <section className="page results-wide">
      <div className="result-top">
        ...
        <div className="big-score">
          <span>{(attempt && attempt.score) || 0}</span>
          <small>/ 100</small>
        </div>
      </div>
      <div className="stat-grid">
        <div>
          <strong>{(attempt && attempt.correct) || 0}</strong>
          <span>Correct</span>
        </div>
        <div>
        ...
        <div>
          <strong>{formatTime((attempt && attempt.timeSpent) || 0)}</strong>
          <span>Time</span>
        </div>
        <div>
      ...
      <div className="review-header">
        <div>
          <h3>Answer review</h3>
          <p>Compare your answer with the correct answer for every question.</p>
        </div>
        <div className="review-filters">
          {['all', 'correct', 'wrong', 'unanswered'].map((item) => (
      ...
      <div className="review-list">
        {shown.map((item) => (
          <article className={`review-card ${item.correct ? 'correct' : 'wrong'}`} key={item.number}>
            <div className="review-card-top">
              <span className="review-number">Q{item.number}</span>
              <span className="review-status">{item.unanswered ? 'Unanswered' : item.correct ? 'Correct' : 'Incorrect'}</span>
            </div>
            <h4>{item.question}</h4>
            <div className="review-options">
              {item.options.map((option, optionIndex) => {
                const selected = item.selected === optionIndex;
                const correct = item.correctIndex === optionIndex;
                return (
                  <div className={`review-option ${selected ? 'selected-answer' : ''} ${correct ? 'correct-answer' : ''}`} key={option}>
                    <span>{String.fromCharCode(65 + optionIndex)}</span>
                    <strong>{option}</strong>
          ...
          {quizzes.map((quiz) => (
            <div className="library-row" key={quiz.id}>
              <button className="library-main" onClick={() => onOpen(quiz)}>
                ...
                <span>
                  {quiz.questions.length} questions · {quiz.timeLimit} min · {(quiz.attempts && quiz.attempts.length) || 0} attempts
                </span>
              </button>
              <button className="delete-button" onClick={() => onDelete(quiz.id)}>
                Delete
              </button>
            </div>
          ))}
...
function AIPage({ onBack }) {
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: "Hi! I'm Kenzy. Ask me to explain a topic, make a study plan, compare concepts, or help you understand something from school.",
    },
  ]);
    ...
    <section className="page ai-page">
      ...
      <div className="ai-heading">
        <div>
          ...
          <h2>Ask Kenzy</h2>
          <p>Get explanations and study guidance without leaving your workspace.</p>
        </div>
      ...
      <div className="chat-card">
        <div className="chat-messages">
          {messages.map((message, index) => (
            <div className={message.role === 'user' ? 'message user' : 'message assistant'} key={`${message.role}-${index}`}>
              <div className="message-avatar">{message.role === 'user' ? 'KC' : 'K'}</div>
        ...
        <form className="chat-form" onSubmit={sendMessage}>
          <textarea value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask Kenzy anything about your studies…" rows={2} />
          <button className="button primary" disabled={!input.trim() || busy}>
            {busy ? 'Thinking…' : 'Send'}
          </button>
        </form>
        <div className="chat-suggestions">
          <button type="button" onClick={() => setInput('Explain this topic like I am a beginner.')}>
            Explain simply
          </button>
          <button type="button" onClick={() => setInput('Help me make a study plan for this week.')}>
            Make a study plan
          </button>
          <button type="button" onClick={() => setInput('Give me practice questions about this topic.')}>
            Practice questions
          </button>
        </div>
      </div>
  ...
  async function improve(action) {
    ...
    try {
      const response = await fetch('/api/study-notes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action, title: draft.title, content: draft.content }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || 'AI notes request failed.');
      setAiResult(data.result || '');
    } catch (error) {
      setAiError(error.message || 'Could not improve the note.');
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="notes-toolbar">
        <div>
          ...
          <div className="section-heading compact">
            ...
            <h2>Your notes</h2>
            <p>Write, save, and ask AI to improve your notes.</p>
          </div>
        </div>
      ...
      <div className="notes-layout">
        <aside className="notes-list">
          ...
          {notes.length === 0 ? (
            ...
            notes.map((note) => (
              <button className={draft.id === note.id ? 'note-row selected' : 'note-row'} key={note.id} onClick={() => onOpen(note)}>
        ...
        <section className="note-editor">
          <input className="note-title" value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} placeholder="Note title" />
          <textarea
          ...
          <div className="editor-actions">
            ...
            {selected && (
              <button className="delete-button" onClick={() => onDelete(selected.id)}>
                Delete
              </button>
            )}
          </div>
          <div className="ai-tools-card">
            <div>
              <div className="eyebrow">NOTE AI</div>
              <h3>Improve this note</h3>
              <p>Use AI to summarize, simplify, or turn your notes into a study guide.</p>
            </div>
            <div className="ai-tool-buttons">
              <button type="button" onClick={() => improve('Summarize these study notes into concise revision points.')}>
                Summarize
              </button>
              <button type="button" onClick={() => improve('Rewrite these notes in simple, beginner-friendly language.')}>
                Simplify
              </button>
              <button type="button" onClick={() => improve('Turn these notes into a clear study guide with headings and key ideas.')}>
                Study guide
              </button>
            </div>
            {busy && <div className="ai-working">✦ Kenzy is working on your notes…</div>}
            {aiError && <div className="error-box">{aiError}</div>}
            {aiResult && (
              <div className="ai-result">
                <div className="result-label">AI RESULT</div>
                <pre>{aiResult}</pre>
              </div>
            )}
          </div>
        ...
        {APPLICATIONS.map((app) => (
          <article className="application-card" key={app.name}>
            ...
            <div className="application-body">
              ...
              <p>{app.description}</p>
              {app.url ? (
                <a className="download-button" href={app.url} target="_blank" rel="noreferrer">
                  Download ↗
                </a>
              ) : (
  ...
  return (
    <section className="page narrow-page">
      ...
      <div className="section-heading">
        ...
        <h2>Customize Kenzy</h2>
        <p>Choose the appearance you prefer. Your setting is saved on this device.</p>
      </div>
      <div className="settings-card">
        <div className="settings-row">
          <div>
            <strong>Appearance</strong>
            <span>Switch between light and high-contrast dark mode.</span>
          </div>
