...
function formatSize(bytes) { if (!bytes) return ''; if (bytes < 1024 * 1024) return `${Math.max(1, Math.round(bytes / 1024))} KB`; return `${(bytes / (1024 * 1024)).toFixed(1)} MB`; }
...
function bindSidebar(sidebar) {
  ...
  sidebar.querySelectorAll('.ai-sidebar-tab').forEach((tab) => tab.addEventListener('click', () => {
...
function refreshSidebar(sidebar) {
  ...
  if (!content) return;
  content.innerHTML = sidebar.dataset.activeTab === 'files' ? renderFiles() : renderPrompts();
  ...
  content.querySelector('[data-add-file]')?.addEventListener('change', (event) => {
    const incoming = Array.from(event.target.files || []).map((file) => ({ id:`${file.name}-${file.size}-${file.lastModified}`, name:file.name, type:file.type || 'File', size:file.size, addedAt:Date.now() }));
    const merged = [...incoming, ...readJson(FILES_KEY)].filter((file,index,all) => all.findIndex((item) => item.id === file.id) === index).slice(0,30);
    writeJson(FILES_KEY, merged);
    refreshSidebar(sidebar);
  });
  content.querySelectorAll('[data-prompt]').forEach((button) => button.addEventListener('click', () => {
...
function renderFiles() {
  ...
  const rows = files.length ? files.map((file) => `<div class="ai-sidebar-row ai-file-row"><span class="ai-row-icon">${file.type.includes('pdf') ? 'PDF' : 'IMG'}</span><span class="ai-row-copy"><strong>${escapeHtml(file.name)}</strong><small>${escapeHtml(file.type)} · ${formatSize(file.size)}</small></span></div>`).join('') : `<div class="ai-sidebar-empty"><span class="ai-empty-icon">▤</span><strong>No files yet.</strong><small>Add study files here and keep them separate from your recent prompts.</small></div>`;
...
function trackPromptFromForm(form) {
  ...
  const next = [{ id:`${Date.now()}-${Math.random().toString(36).slice(2)}`, title:prompt.length > 48 ? `${prompt.slice(0,48)}…` : prompt, prompt, createdAt:Date.now() }, ...current.filter((item) => item.prompt !== prompt)].slice(0,30);
  writeJson(PROMPTS_KEY, next);
  const sidebar = document.getElementById(SIDEBAR_ID);
  if (sidebar?.dataset.activeTab === 'prompts') refreshSidebar(sidebar);
}
...
function setup() {
  ...
  document.addEventListener('submit', (event) => {
    if (event.target instanceof HTMLFormElement && event.target.matches('.chat-form')) window.setTimeout(() => trackPromptFromForm(event.target), 0);
  }, true);
  const observer = new MutationObserver(() => {
    const hasAiPage = Boolean(document.querySelector('.ai-page'));
    const sidebar = document.getElementById(SIDEBAR_ID);
    if (hasAiPage && !sidebar) renderSidebar();
    if (!hasAiPage && sidebar) sidebar.remove();
  });
