...
// Gemini 2.5 is unavailable to some new users. Use current models only.
const GEMINI_MODELS = [
  'gemini-3.6-flash',
  'gemini-3.5-flash',
];
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`, {
          ...
          body: JSON.stringify({
            ...
            generationConfig: { maxOutputTokens: 1600, temperature: 0.45 },
          }),
        });
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          const response = await ai.models.generateContent({
            model,
            contents,
            config: { systemInstruction: SYSTEM_PROMPT, maxOutputTokens: 1800, temperature: 0.45 },
          });
  ...
  try {
    ...
    if (blobFiles.length) {
      ...
      for (const blobFile of blobFiles.slice(0, 8)) {
        ...
        resolved.push({ name: blobFile.name, mimeType: blobFile.mimeType, data: buffer.toString('base64') });
