...
async function generateWithGeminiFiles(apiKey, messages, files) {
  ...
  try {
    for (const file of files) {
      ...
      let status = item;
      for (let attempt = 0; attempt < 20 && status?.state === 'PROCESSING'; attempt += 1) { await sleep(500); status = await ai.files.get({ name: item.name }); }
    ...
    for (const model of GEMINI_MODELS) {
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          const response = await ai.models.generateContent({ model, contents, config: { maxOutputTokens: 900 } });
  ...
  try {
    ...
    if (blobFiles.length) {
      ...
      for (const blobFile of blobFiles.slice(0, 8)) {
        ...
        resolved.push({ name: blobFile.name, mimeType: blobFile.mimeType, data: buffer.toString('base64') }); cleanup.push(pathname);
      }
    ...
    const reply = needsFilesApi ? await generateWithGeminiFiles(apiKey, cleanMessages, safeFiles) : await generateText(apiKey, cleanMessages);
