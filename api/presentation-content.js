...
async function askGemini(apiKey, kind, body, files) {
  ...
  try {
    ...
    const config = kind === 'quiz'
      ? { responseMimeType: 'application/json', maxOutputTokens: Math.min(32768, Math.max(8192, count * 650)), thinkingConfig: { thinkingLevel: 'low' } }
      : { maxOutputTokens: kind === 'notes' ? 3500 : 800, thinkingConfig: { thinkingLevel: 'low' } };
