  ...
  try {
    ...
    if (blobFiles.length) {
      if (!process.env.BLOB_READ_WRITE_TOKEN) {
        return res.status(503).json({ error: 'Large note imports are not configured. Connect the Vercel Blob store first.' });
      }
      let total = 0;
      files = [];
    ...
    if (files.some((file) => !file || !ALLOWED.has(file.mimeType) || typeof file.data !== 'string' || !file.data)) {
      return res.status(400).json({ error: 'Use PDF, PNG, JPG, or WebP files only.' });
    }
    ...
    const prompt = [
      ...
      files.length ? 'The uploaded study file is the primary source for this request. Extract and organize its useful study content.' : '',
      'For an import task, return clean editable notes rather than a summary about the file.',
    ].filter(Boolean).join('\n\n');
