  ...
  try {
    ...
    if (blobFiles.length) {
      ...
      for (const item of blobFiles) {
        ...
        files.push({ name: item.name || 'study-file', mimeType, data: buffer.toString('base64') }); cleanup.push(pathname);
      }
    }
    ...
    const prompt = [`Task: ${action}`, title ? `Note title: ${title}` : '', content.trim() ? `Current note:\n${content}` : '', files.length ? 'The uploaded file is the primary source. Read it carefully and create accurate, editable study notes. Preserve the source structure and important details.' : '', 'Be concise but complete. Finish the requested task in one response.'].filter(Boolean).join('\n\n');
    const result = files.some((file) => PPT_TYPES.has(file.mimeType)) || totalBytes > 3 * 1024 * 1024 ? await callGeminiFilesApi(apiKey, prompt, files, action) : await callGemini(apiKey, prompt, files, action);
    ...
    if (cleanup.length && process.env.BLOB_READ_WRITE_TOKEN) { try { const { del } = await import('@vercel/blob'); await Promise.allSettled(cleanup.map((pathname) => del(pathname, { token: process.env.BLOB_READ_WRITE_TOKEN }))); } catch {} }
  }
}
